        ...
        try:
            ...
            async with session.get(url, params=params, headers=HEADERS_WF, timeout=aiohttp.ClientTimeout(total=30)) as r:
                if r.status in (429, 500, 502, 503, 504):
                    await asyncio.sleep(1.2 * (i + 1)); continue
                r.raise_for_status()
                return await r.json()
        except Exception:
...
def sum_24h_volume(stats_json):
    if not stats_json: return 0
    payload = stats_json.get("payload") or {}
    stats = payload.get("statistics_48hours") or (payload.get("statistics") or {}).get("48hours")
    if not isinstance(stats, list): return 0
    cutoff = datetime.now(timezone.utc) - timedelta(hours=24)
    total = 0
    for row in stats:
        ts = row.get("datetime") or row.get("date") or row.get("time")
        try: dt = datetime.fromisoformat(ts.replace("Z", "+00:00")) if ts else None
        except Exception: dt = None
        if dt and dt >= cutoff:
            vol = row.get("volume")
            try: total += int(vol) if vol is not None else 0
            except Exception: pass
    return total

def count_ingame_orders(orders_json):
    if not orders_json: return {"buy": 0, "sell": 0}
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, entry in enumerate(top_for_depth, 1):
            ...
            enriched.append({"item": entry["item"], "volume24": entry["volume24"], "buy_orders": depth["buy"], "sell_orders": depth["sell"]})
            if i % 40 == 0:
        ...
        for e in enriched:
            ...
            depth_s = (float(e["buy_orders"] + e["sell_orders"]) / float(max_depth)) if max_depth else 0.0
            score = 0.7 * vol_s + 0.3 * depth_s
            scored.append({"item": e["item"], "volume24": e["volume24"], "buy_orders": e["buy_orders"], "sell_orders": e["sell_orders"], "score": round(score, 6)})
        ...
        r = requests.post(SUPABASE_URL + "/rest/v1/tracked_items?on_conflict=item", headers={
            "apikey": SUPABASE_KEY, "Authorization": "Bearer " + str(SUPABASE_KEY),
            "Content-Type": "application/json", "Prefer": "return=minimal,resolution=merge-duplicates"
        }, data=json.dumps(payload), timeout=30)
        if not r.ok:
            print("tracked_items upsert failed: {0} {1}".format(r.status_code, r.text))
            raise SystemExit(1)
        print("Done. Examples:")
        for e in top[:10]:
            depth_sum = e["buy_orders"] + e["sell_orders"]
            print(" - {0:>24}  score={1:.3f}  vol24={2}  depth={3}".format(e["item"], e["score"], e["volume24"], depth_sum))
