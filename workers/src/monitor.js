...
async function sendDiscordAlert(env, alert) {
    ...
    const embed = {
        ...
        fields: alert.fields || [], timestamp: new Date().toISOString(), footer: { text: "Gandalf System Monitor" }
    };
    try {
...
async function checkSystemHealth(env) {
    ...
    const dataTimestamp = Array.isArray(data) && data[0]?.timestamp ? new Date(data[0].timestamp).getTime() : new Date(data.timestamp).getTime();
    const age = Date.now() - dataTimestamp;
    if (age > 1800000) { return { healthy: false, issue: `Data is ${Math.round(age / 60000)} minutes old` }; }
    try {
        const response = await fetch(`${env.SUPABASE_URL}/rest/v1/market_data?limit=1`, { headers: { 'apikey': env.SUPABASE_ANON_KEY } });
        if (!response.ok) { return { healthy: false, issue: "Supabase connection failed" }; }
    } catch (error) { return { healthy: false, issue: "Database connection error" }; }
